...
import { THEME_MODES, THEME_STORAGE_KEY, applyThemeMode, getStoredThemeMode, getThemeMediaQuery, type ThemeMode } from '../theme'
...
const layerControlConfigs: Array<SliderConfig<LayerNumericKey>> = [
  { key: 'gain', label: 'Layer gain', min: STUDIO_LIMITS.layerGain.min, max: STUDIO_LIMITS.layerGain.max, step: STUDIO_LIMITS.layerGain.step, format: (value) => `${Math.round(value * 100)}%` },
  { key: 'pan', label: 'Pan', min: STUDIO_LIMITS.pan.min, max: STUDIO_LIMITS.pan.max, step: STUDIO_LIMITS.pan.step, format: (value) => `${value > 0 ? '+' : ''}${Math.round(value * 100)}` },
  { key: 'noise', label: 'Noise mix', min: STUDIO_LIMITS.noise.min, max: STUDIO_LIMITS.noise.max, step: STUDIO_LIMITS.noise.step, format: (value) => `${Math.round(value * 100)}%` },
  { key: 'startFreq', label: 'Start pitch', min: STUDIO_LIMITS.frequency.min, max: STUDIO_LIMITS.frequency.max, step: STUDIO_LIMITS.frequency.step, format: formatFrequency },
  { key: 'endFreq', label: 'End pitch', min: STUDIO_LIMITS.frequency.min, max: STUDIO_LIMITS.frequency.max, step: STUDIO_LIMITS.frequency.step, format: formatFrequency },
  { key: 'detuneCents', label: 'Detune', min: STUDIO_LIMITS.detuneCents.min, max: STUDIO_LIMITS.detuneCents.max, step: STUDIO_LIMITS.detuneCents.step, format: (value) => `${Math.round(value)} cents` },
  { key: 'startMs', label: 'Start offset', min: STUDIO_LIMITS.layerStartMs.min, max: STUDIO_LIMITS.layerStartMs.max, step: STUDIO_LIMITS.layerStartMs.step, format: formatMilliseconds },
  { key: 'durationMs', label: 'Layer duration', min: STUDIO_LIMITS.layerDurationMs.min, max: STUDIO_LIMITS.layerDurationMs.max, step: STUDIO_LIMITS.layerDurationMs.step, format: formatMilliseconds },
  { key: 'vibratoDepth', label: 'Vibrato depth', min: STUDIO_LIMITS.vibratoDepth.min, max: STUDIO_LIMITS.vibratoDepth.max, step: STUDIO_LIMITS.vibratoDepth.step, format: formatFrequency },
  { key: 'vibratoRate', label: 'Vibrato rate', min: STUDIO_LIMITS.vibratoRate.min, max: STUDIO_LIMITS.vibratoRate.max, step: STUDIO_LIMITS.vibratoRate.step, format: (value) => `${value.toFixed(1)} Hz` },
  { key: 'transient', label: 'Transient', min: STUDIO_LIMITS.transient.min, max: STUDIO_LIMITS.transient.max, step: STUDIO_LIMITS.transient.step, format: (value) => `${Math.round(value * 100)}%` },
]

const envelopeControlConfigs: Array<SliderConfig<EnvelopeNumericKey>> = [
  { key: 'attackMs', label: 'Attack', min: STUDIO_LIMITS.attackMs.min, max: STUDIO_LIMITS.attackMs.max, step: STUDIO_LIMITS.attackMs.step, format: formatMilliseconds },
  { key: 'holdMs', label: 'Hold', min: STUDIO_LIMITS.holdMs.min, max: STUDIO_LIMITS.holdMs.max, step: STUDIO_LIMITS.holdMs.step, format: formatMilliseconds },
  { key: 'decayMs', label: 'Decay', min: STUDIO_LIMITS.decayMs.min, max: STUDIO_LIMITS.decayMs.max, step: STUDIO_LIMITS.decayMs.step, format: formatMilliseconds },
  { key: 'sustain', label: 'Sustain', min: STUDIO_LIMITS.sustain.min, max: STUDIO_LIMITS.sustain.max, step: STUDIO_LIMITS.sustain.step, format: (value) => `${Math.round(value * 100)}%` },
  { key: 'releaseMs', label: 'Release', min: STUDIO_LIMITS.releaseMs.min, max: STUDIO_LIMITS.releaseMs.max, step: STUDIO_LIMITS.releaseMs.step, format: formatMilliseconds },
]

const filterControlConfigs: Array<SliderConfig<FilterNumericKey>> = [
  { key: 'cutoffHz', label: 'Cutoff', min: STUDIO_LIMITS.cutoffHz.min, max: STUDIO_LIMITS.cutoffHz.max, step: STUDIO_LIMITS.cutoffHz.step, format: formatFrequency },
  { key: 'resonance', label: 'Resonance', min: STUDIO_LIMITS.resonance.min, max: STUDIO_LIMITS.resonance.max, step: STUDIO_LIMITS.resonance.step, format: (value) => value.toFixed(1) },
  { key: 'envelopeAmount', label: 'Env amount', min: STUDIO_LIMITS.envelopeAmount.min, max: STUDIO_LIMITS.envelopeAmount.max, step: STUDIO_LIMITS.envelopeAmount.step, format: (value) => `${value.toFixed(1)} oct` },
]

const masterControlConfigs: Array<SliderConfig<MasterNumericKey>> = [
  { key: 'gain', label: 'Master gain', min: STUDIO_LIMITS.masterGain.min, max: STUDIO_LIMITS.masterGain.max, step: STUDIO_LIMITS.masterGain.step, format: (value) => `${Math.round(value * 100)}%` },
  { key: 'drive', label: 'Drive', min: STUDIO_LIMITS.drive.min, max: STUDIO_LIMITS.drive.max, step: STUDIO_LIMITS.drive.step, format: (value) => `${Math.round(value * 100)}%` },
  { key: 'delayMix', label: 'Delay mix', min: STUDIO_LIMITS.delayMix.min, max: STUDIO_LIMITS.delayMix.max, step: STUDIO_LIMITS.delayMix.step, format: (value) => `${Math.round(value * 100)}%` },
  { key: 'delayMs', label: 'Delay time', min: STUDIO_LIMITS.delayMs.min, max: STUDIO_LIMITS.delayMs.max, step: STUDIO_LIMITS.delayMs.step, format: formatMilliseconds },
  { key: 'delayFeedback', label: 'Feedback', min: STUDIO_LIMITS.delayFeedback.min, max: STUDIO_LIMITS.delayFeedback.max, step: STUDIO_LIMITS.delayFeedback.step, format: (value) => `${Math.round(value * 100)}%` },
  { key: 'stereoWidth', label: 'Stereo width', min: STUDIO_LIMITS.stereoWidth.min, max: STUDIO_LIMITS.stereoWidth.max, step: STUDIO_LIMITS.stereoWidth.step, format: (value) => `${value.toFixed(2)}x` },
]
...
function StudioPage({
  ...
  const [status, setStatus] = useState(
    importedPreset
      ...
      : 'Build with layers, then release a slider or hit Play to audition the patch.',
  )
  ...
  const availableSources = useMemo(
    () => [
      ...STUDIO_PATCHES.map((preset) => ({ id: preset.id, name: preset.name, description: preset.description, source: 'studio' as const })),
      ...PRESETS.map((preset) => ({ id: `simple-${preset.id}`, name: `${preset.name} -> Studio`, description: preset.description, source: 'simple' as const })),
    ],
    [],
  )
  ...
  const waveformPath = useMemo(() => {
    ...
    return toWaveformPath(mixStereoForDisplay(render), 840, 240)
  }, [patch])
  ...
  function updatePatchName(name: string) {
    commitPatch({ ...patchRef.current, name }, { status: `Renamed patch to ${name || 'Untitled Patch'}.` })
  }
  ...
  function updateSelectedLayer(updater: (layer: StudioLayer) => StudioLayer, options: { preview?: boolean; status?: string } = {}) {
  ...
  function handleFilterTypeChange(type: StudioFilterType) {
    updateSelectedLayer((layer) => ({ ...layer, filter: { ...layer.filter, type } }), { preview: true })
  }
  ...
  function handleAddLayer() {
    ...
    const nextLayer = createStudioLayer(createLayerId(), `Layer ${patchRef.current.layers.length + 1}`, {
  ...
  function handleRemoveLayer() {
    if (!selectedLayer || patchRef.current.layers.length === 1) {
    ...
    setSelectedLayerId(nextLayers[0]?.id ?? 'layer-1')
    commitPatch({ ...patchRef.current, layers: nextLayers }, { status: `Removed ${selectedLayer.name}.` })
  }
    ...
    <main className="studio-page-shell">
      <section className="studio-topbar">
        ...
        <div className="studio-topbar__actions">
          ...
          <button type="button" className="studio-toolbar-button" onClick={handleRandomizeSelectedLayer}>
            Randomize layer
          </button>
          <button type="button" className="studio-toolbar-button studio-toolbar-button--accent" onClick={handleExport}>
            Export stereo WAV
          </button>
          <button
            type="button"
      ...
      <section className="studio-workspace">
        <aside className="studio-sidebar studio-sidebar--left">
          ...
          <div className="studio-source-list" aria-label="Studio patch browser">
          ...
          <div className="studio-panel-head studio-panel-head--layers">
            <div>
              <p className="studio-panel-kicker">Layers</p>
              <h2>Patch stack</h2>
            </div>
            <span>
              {patch.layers.length}/{MAX_STUDIO_LAYERS}
            </span>
          </div>
          ...
          <div className="studio-layer-actions">
            ...
            <button type="button" className="studio-toolbar-button" onClick={handleRemoveLayer}>
              Delete
            </button>
          </div>

          <div className="studio-layer-list" role="list" aria-label="Patch layers">
        ...
        <section className="studio-center-panel">
          <div className="studio-hero-card">
            ...
            <div className="studio-fact-row">
              ...
              <div>
                <span>Layers</span>
                <strong>{patch.layers.length}</strong>
              </div>
              <div>
                <span>Selected</span>
          ...
          <div className="studio-wave-card">
            ...
            <svg className="studio-wave" viewBox="0 0 840 240" role="img" aria-label="Studio waveform preview">
              <defs>
              ...
              <path className="studio-wave__ghost" d="M0 120 L840 120" />
              <path className="studio-wave__line" d={waveformPath} />
            </svg>

            <div className="studio-wave-meta">
              <span>{selectedLayer ? `${selectedLayer.name}: ${formatFrequency(selectedLayer.startFreq)} to ${formatFrequency(selectedLayer.endFreq)}` : 'No layer selected.'}</span>
              <span>{patch.master.delayMix > 0 ? `Delay ${Math.round(patch.master.delayMix * 100)}%` : 'Dry patch'}</span>
        ...
        <aside className="studio-sidebar studio-sidebar--right">
          {selectedLayer ? (
            <>
              <section className="studio-inspector-card">
                <div className="studio-panel-head">
                  <div>
